<template>
  <div class="warp">
    <div class="bind-printer" v-if="isPrinter">
      <p class="title">
        <span>我的打印机</span>
        <i class="el-icon-more-outline more bind-more" @click="togglePrinter"></i>
      </p>
      <div class="printer-Info">
        <span>当前打印机：</span>
        <div class="printer-Info-box">
          <p class="printer-Info-item">
            <i class="el-icon-printer" style="font-size: 24px;color: #f5f5f5;"></i>：
            <span v-if="printerValue">{{printerValue.model||'null'}}</span></p>
          <p class="printer-Info-item">
            <i class="el-icon-tickets" style="font-size: 24px;color: #f5f5f5;"></i>：
            <span v-if="printerValue">{{printerValue.number||'number为空'}}</span></p>
        </div>
      </div>
      <div class="printer-insert">
        <el-input v-model.trim="number" style="margin-right:10px;width: 64%;" maxlength="16" size="small"
                  laceholder="请输入打印机编号" clearable/>
        <el-button type="danger" size="small" @click="bindPrintNumber" plain v-throttle>绑定</el-button>
      </div>
    </div>
    <div class="bind-printer toggle-printer" v-if="!isPrinter">
      <p class="title">
        <span>选择打印机</span>
        <i class="el-icon-more-outline more toggle-more" @click="togglePrinter"></i>
      </p>
      <div class="toggle-printer-Info">
        <div class="toggle-Info-item" :class="{isActive:index==activeIndex}"
             v-for="(item,index) in  printerList" :key="index"
             @click="selectPrint(index)">
          <i class="el-icon-printer" style="font-size: 20px;color: #f5f5f5;"></i>&nbsp;
          <span>{{item.number}}</span>
        </div>
      </div>
      <div class="toggle-printer-footer">
        <el-button size="mini" @click="deletePrinter" type="danger" plain>删除</el-button>
        <el-button size="mini" @click="confirm" type="primary" plain>选中</el-button>
      </div>
    </div>
  </div>
